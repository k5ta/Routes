...
Данная задача является NP-трудной, а это значит, что, с учетом распространенной, но недоказанной гипотезы о неравенстве классов P и NP, нельзя найти ее решение за полиномиальное от количества городов времени.
...
Рассмотрим сложности частей алгоритма. Так, подготовка матрицы размера n на n - **O(n^3)**, так как за O(n) мы находим минимальный элемент в строке, за O(n) вычитаем его из всех элементов строки, а также O(n) требуется для выполнения этого для всех строк. Аналогично для столбцов, поэтому получаем O(n^3 + n^3) = O(n^3)

Поиск нулей в матрице n на n - **O(n^2)**. Также **O(n^2)** для подсчета коэффициента для одного из нулей. Сокращение матрицы и добавление вершин в текущий ответ - **O(n)**

Таким образом, одна итерация для матрицы размера n на n имеет сложность **O(n^3)**.

## Оценка использования памяти основными алгоритмами
#### Для метода грубой силы имеем:
Используемые для получения всех вариантов пути (n-1)! перестановок вершин (без начальной) нам хранить не трубется, так как в данном случае будем использовать генератор, размер которого чуть менее, чем в два раза больше размера массива из n-1 элеметов. Поэтому для хранения генератора нужно **O(2n) = O(n)** памяти, также **O(n)** для хранения текущего пути на каждой из итераций, а кроме того константная память **O(1)** для хранения текущей минимальной длины и **O(n)** - для текущего лучшего маршрута. Таким образом, метод грубой силы расходует **O(n)** памяти.

#### Для алгоритма Литтла имеем:
Так как алгоритм сначала находит одно решение, а при дальнейшем поиске идет сравнение текущей границы с полученной нижней границей, то в худшим случаем будет такой, когда перед подсчетом решения на каждой из итераций идет ветвление, из-за которого происходит копирование текущей матрицы. Таким образом, с учетом занимания матрицей размера n на n памяти O(n^2), занимаемая в этом случае память **O(n^2) + (n-1)^2 + ... + 2^2)** (так как для матрицы размера 2x2 копирования не будет). Поскольку сумма квадратов от 0 до n равна 1/6*n(n+1)(2n+1), это даст сложность **O(n^3)**.
